...
func TestWebsocketV5Public_Ticker(t *testing.T) {
	...
	t.Run("linear_snapshot_with_funding", func(t *testing.T) {
		respBody := map[string]interface{}{
			"topic": "tickers.BTCUSDT",
			"type":  "snapshot",
			"data": map[string]interface{}{
...

		{
			_, err := svc.SubscribeTicker(
				...
				func(response V5WebsocketPublicTickerResponse) error {
					assert.Equal(t, respBody["topic"], response.Topic)
					testhelper.Compare(t, respBody["data"], response.Data.LinearInverse)
					assert.Equal(t, "8", response.Data.LinearInverse.FundingIntervalHour)
					assert.Equal(t, "0.005", response.Data.LinearInverse.FundingCap)
					return nil
				},
			)
			require.NoError(t, err)
	...
	t.Run("linear_delta_without_funding", func(t *testing.T) {
		respBody := map[string]interface{}{
			"topic": "tickers.BTCUSDT",
			"type":  "delta",
			"data": map[string]interface{}{
...

		{
			_, err := svc.SubscribeTicker(
				...
				func(response V5WebsocketPublicTickerResponse) error {
					assert.Equal(t, respBody["topic"], response.Topic)
					testhelper.Compare(t, respBody["data"], response.Data.LinearInverse)
					assert.Equal(t, "", response.Data.LinearInverse.FundingIntervalHour)
					assert.Equal(t, "", response.Data.LinearInverse.FundingCap)
					return nil
				},
			)
			require.NoError(t, err)
